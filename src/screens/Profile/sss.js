...
import { POPPINS_REGULAR, POPPINS_SEMIBOLD, SCREEN_WIDTH, STANDARD_BORDER_RADIUS, STANDARD_SPACING } from '../../constants/constants';
import TextInputComp from '../../components/Input/TextInputComp';
import { Formik } from 'formik';
import * as Yup from 'yup';
import { Findaddress, GetBusinessType, GetIndustryType, savePhoto ,BussinessUpdateProfile } from '../../api/auth/auth';
...
const Profile = ({ navigation }) => {
  ...
  const [userProfilePic, setUserProfilePic] = useState(userInfo?.data?.profile_image);
  ...
  const openCamera = async () => {
    ...
    if (
      ...
      const dataggg = { profile_image: uri };
      ...
      await savePhoto(userInfo.token.access, formData)
        .then(val => {
          ...
          dispatch(updateUser(val?.user_profile_data?.profile_image));
          // var data = { profilePic: val?.result?.Image?.url };
          // updateUserDatels(token, data);
  ...
  const handleSubmit = async values => {
    ...
    await BussinessUpdateProfile(userInfo.token.access, data)
      .then(responseData => {
        ...
        dispatch(
          updateUser({...userInfo.business_profile_data, ...responseData.data}),
        );
      ...
      <ScrollView>
        <View style={styles.Imagecontainer}>
          <Image
            ...
            source={{ uri:  `${API_URL}userProfilePic` }}
            // source={{ uri: cameraPhoto ? cameraPhoto : userProfilePic }}
          ...
          <View style={styles.usernameview}>
            <Text style={styles.usernametxt}>Heylie ronaldo</Text>
          </View>
        </View>
        <View style={{ marginHorizontal: scale(0) }}>
          <Formik
            initialValues={{
              ...
              industryName: '',
              businessType: userInfo.business_profile_data.business_type || '',
              businessName: '',
            }}
                            ...
                            onPress={() => {
                              ...
                              if (zipCode === "") {
                                ...
                                showMessage({
                                  ...
                                  description: 'Please enter the 6 digit ZIP code to proceed.',
                                  type: 'warning',
                                });
                              } else {
                      ...
                      <View style={styles.inputview}>
                        <View style={styles.equalInput}>
                          ...
                          <TextInputComp
                            ...
                            inputStyle={{ backgroundColor: autoFilled ? colors.gray : null }}
                          />
