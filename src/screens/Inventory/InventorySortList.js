...
const InventorySortList = ({navigation, route,index}) => {
  ...
  const datafilter = async data => {
    ...
    await userSortFilter(userInfo.token.access, isrouteData, page)
      .then(res => {
        console.log('Products Found Successfully!', res?.data?.length);
        setSortingData(pre => [...pre, ...res.data]);
...
export default InventorySortList;
