...
const LogBookSortList = ({navigation, route}) => {
  ...
  const datafilter = async data => {
    ...
    await ledgerBookSort(userInfo.token.access, isrouteData, page)
      .then(res => {
        console.log('Products Found Successfully!', res);
        // const pkpk = [...filterSortData, ...res.data];
        // const filteredData = res.data.filter(item => parseFloat(item.all_grand_total) !== 0 && parseFloat(item.all_remaining) !== 0);
        // setFilterSortData(pre => [...pre, ...filteredData]);
  ...
  const renderItem = ({item, index}) => {
    const status =
      item.sort_type == 'debt' ? 2 : item.sort_type == 'sales' ? 3 : item.last_invoice_status;
    const price =
      item.sort_type == 'debt'
        ? item.all_remaining
        : item.sort_type == 'sales'
        ? item.all_grand_total
        : item.last_invoice_grand_total;
    return (
      <LedgerList
        status={status}
        price={price}
...
export default LogBookSortList;
